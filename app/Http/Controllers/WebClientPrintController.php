    ...
    public function processRequest(Request $request){
        ...
        try{
            ...
            else if ($reqType == WebClientPrint::ClientSetInstalledPrinters)
            {
                ...
                WebClientPrint::cacheAdd($sid, WebClientPrint::WCP_CACHE_PRINTERS, strlen($request->input(WebClientPrint::WCPP_SET_PRINTERS)) > 0 ? base64_decode($request->input(WebClientPrint::WCPP_SET_PRINTERS)) : '');
                return;
            }
            ...
            else if ($reqType == WebClientPrint::ClientGetInstalledPrinters)
            {
                ...
                return response(WebClientPrint::cacheGet($sid, WebClientPrint::WCP_CACHE_PRINTERS))
                        ->header('Content-Type', 'text/plain'); 
            }    
        }
        catch (Exception $ex)
